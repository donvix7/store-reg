...
import { CheckCheckIcon, TriangleAlert, BellRing, Filter, ChevronDown } from 'lucide-react';
...
const NotificationList = ({ notifications }) => {
    ...
    const renderIcon = (name) => {
        switch (name) {
            ...
            case 'CheckCheck': return <CheckCheckIcon size={20} />;
            default: return <CheckCheckIcon size={20} />;
        }
    };
  ...
  return (
    <div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="col-span-1 md:col-span-12 lg:col-span-8 bg-surface-container-low p-8 rounded-xl flex justify-between items-center overflow-hidden relative">
          <div className="relative z-10">
            <h3 className="font-headline text-3xl font-bold mb-2">System Health</h3>
            <p className="text-on-surface-variant max-w-md">All systems are operational. You have 3 urgent stock alerts that require immediate attention.</p>
        ...
        <div className="col-span-1 md:col-span-12 lg:col-span-4 bg-tertiary-fixed p-8 rounded-xl flex flex-col justify-center">
          <p className="text-on-tertiary-fixed font-bold uppercase tracking-widest text-xs mb-2">Urgent Tasks</p>
          <span className="font-headline text-5xl font-extrabold text-on-tertiary-fixed">03</span>
      ...
      <section className="bg-surface-container-lowest rounded-xl shadow-sm overflow-hidden border border-outline-variant/10">
        ...
        <div className="divide-y divide-outline-variant/10">
          ...
          {(notifications || [])
            ...
            <div key={index} className={`p-8 flex flex-col sm:flex-row items-start gap-6 hover:bg-surface-container-low transition-colors group cursor-pointer ${notification.status === 'read' ? 'opacity-60' : ''}`}>
              ...
              <div className="flex-1 space-y-1">
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1 text-[10px] font-bold rounded-full uppercase tracking-widest ${notification.badgeClass}`}>{notification.badge}</span>
                  {notification.status === 'unread' && (
                    <span className={`w-1.5 h-1.5 rounded-full ${notification.dotClass}`}></span>
                  )}
                </div>
                <h4 className="font-headline text-lg font-bold text-on-surface">{notification.type}: {notification.sku}</h4>
                <p className="text-on-surface-variant leading-relaxed">{notification.message}</p>
              </div>
              <div className="text-left sm:text-right mt-4 sm:mt-0 w-full sm:w-auto flex flex-row sm:flex-col justify-between sm:justify-start items-center sm:items-end">
                <p className="text-sm font-medium text-outline">{notification.date}</p>
                {notification.status === 'unread' ? (
