"use client"
import { AlertTriangle, ArrowRight, Calendar, ChevronDown, ChevronLeft, ChevronRight, Download, Package, Plus, Search } from 'lucide-react';
...
const DesktopInventory = ({initialInventory}) => {
  ...
  return (
    <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 overflow-hidden">
       {/* Header Section */}
      <div className="hidden lg:flex justify-between items-end p-8">
        <div>
          <h2 className="font-headline text-3xl font-extrabold text-on-surface tracking-tight">Product Inventory</h2>
          <p className="text-on-surface-variant mt-1">Manage and track your global warehouse stock levels.</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6 px-8 mb-8">
        <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/10">
          <p className="text-outline text-xs font-bold uppercase tracking-widest mb-4">Total SKUs</p>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-headline font-bold text-on-surface">{totalCount}</span>
          </div>
        </div>
        <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/10">
          <p className="text-outline text-xs font-bold uppercase tracking-widest mb-4">Optimal Stock</p>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-headline font-bold text-on-surface">{optimalStock}</span>
          </div>
        </div>
        <div className="bg-tertiary-fixed p-6 rounded-xl">
          <p className="text-on-tertiary-fixed-variant text-xs font-bold uppercase tracking-widest mb-4">Low Stock Alerts</p>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-headline font-bold text-on-tertiary-fixed">{lowStock}</span>
            <AlertTriangle className="text-on-tertiary-fixed" size={18} />
          </div>
        </div>
